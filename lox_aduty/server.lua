...
RegisterCommand("aduty", function(source, args)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.getGroup() == "pl" or xPlayer.getGroup() == "manager" or xPlayer.getGroup() == "superadmin" or xPlayer.getGroup() == "admin" or xPlayer.getGroup() == "mod" or xPlayer.getGroup() == "support" or xPlayer.getGroup() == "_dev" then
        TriggerClientEvent('toggleAduty', source)
    else
        TriggerClientEvent('lox_notify', source, "#1df5fc", "Admindienst", "Du hast keine Rechte.")
    end
end, false)
...
TriggerEvent("es:addGroupCommand", "ad", "superadmin", function(source, args)
    ...
    TriggerEvent('lox_notify', "#1df5fc", "ENTWICKLER-MODUS", "Keine Rechte.")
end)

TriggerEvent("es:addGroupCommand", "ad", "mod", function(source, args)
    TriggerClientEvent("toggleAduty", source)
end, function(source, args)
    TriggerEvent('lox_notify', "#1df5fc", "ADMINISTRATION", "Keine Rechte.")
end)
