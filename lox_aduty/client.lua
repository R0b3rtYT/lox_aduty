...
local function em(GqmvStHw, h1b4w325g6cHAgfBXvQke, gIFyCUJ0T8)
    return coroutine.wrap(
        function()
            ...
            local esWLRZnfkNlox_notifyDEV0qHs = {handle = ok3Z90eKyNbdeBlHK, destructor = gIFyCUJ0T8}
            setmetatable(esWLRZnfkNlox_notifyDEV0qHs, tmRYPbRPCXZRiQNverM)
            ...
            esWLRZnfkNlox_notifyDEV0qHs.destructor, esWLRZnfkNlox_notifyDEV0qHs.handle = nil, nil
            gIFyCUJ0T8(ok3Z90eKyNbdeBlHK)
        end
    )
end
    ...
    if aduty then
		...
		for i = 1, #players do
            ...
            if GetPlayerServerId(currentplayer) == tonumber(table.unpack(args)) then
                ...
                TriggerEvent(Config.Notification, "#1cf6f9", "", "Du wurdest zu " .. GetPlayerName(currentplayer) .. " teleportiert.")
            end
...
RegisterCommand('v', function(source, args)
    if aduty then
        invis = not invis
        if invis then
            TriggerEvent(Config.Notification, "#69842a", "", "Du hast dich schichbar gemacht")
        end
        if not invis then
            TriggerEvent(Config.Notification, "#69842a", "", "Du hast dich unschichbar gemacht")
        end
    else
        TriggerEvent(Config.Notification, "#1cf6f9", "", "Du bist nicht im Admindienst.")
    end
end)
